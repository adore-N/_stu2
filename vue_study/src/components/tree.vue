<template>
  <div class="ztree_content_wrap" v-if='treeDataSource.length>0'>
        <div class="zTreeDemoBackground left">
            <ul class="ztree">
                <ztree-item v-for='(item,index) in treeDataSource' :key='index'
                :model.sync='item'
                :num.sync='index'
                :isCheck='isCheck'
...
export default {
   ...
   methods: {
      // 初始化数据
      initTreeData() {
         var tempList = JSON.parse(JSON.stringify(this.dataList));
         // 递归操作，遍历数据，添加对应属性，使传递给子组件的  // 是否可以选择
         var recurrenceFunc = (data) => {
            data.map((item) => {
               // 是否需要复选框
               if(!item.hasOwnProperty('isCheck')){
                  item.isCheck = item.hasOwnProperty('isCheck') ? item.isCheck : this.isCheck;
               }
               if(!item.hasOwnProperty('isOpen')){
                  item.isOpen = item.hasOwnProperty('isOpen') ? item.isOpen : this.isOpen
               }
               // 展开按钮的样式控制
               if(!item.hasOwnProperty('ckbool')){
                  item.ckbool = item.hasOwnProperty('ckbool') ? item.ckbool : false;
               }
               // 控制子级的展开和收缩
               if(!item.hasOwnProperty('isFolder')){
                  item.isFolder = item.hasOwnProperty('open') ? item.open : this.isOpen
               }
               item.children = item.children || [];
               item.loadNode = 0;
               recurrenceFunc(item.children);
            })
      ...
      ztreeItem: {
         ...
         methods:{
            ...
            open(model){
               ...
               if(typeof this.expandfunc == "function" && model.isExpand) {
                  if(mmodel.loadNode!=2) {
                     //
                        this.expandfunc.call(null,model);
                     }else {
         ...
         `<li :class="liClassVal">
               ...
               <ul :class="ulClassVal" v-show='model.isFolder'>
                  <ztree-item v-for="(item,index) in model.children" :key='index' :isCheck='isCheck'
                  :model.sync='item'
                  :num='index'
                  root='1'
